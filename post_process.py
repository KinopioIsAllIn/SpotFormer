...
def calculate_proposal_with_score(array_score_start, array_score_end,
    ...
    ret = None
    if type_idx == 2:
        STEP = int(opt["micro_average_len"] // 2)  # int(opt["micro_average_len"]*3/2)
    ...
    if apex_indices.size > 0:
        ...
        if len(start_indices_list) > 0:
            ...
            ret = (np.array([
                            ...
                            start_indices, end_indices, apex_indices,
...
def nms(df, opt):
    ...
    tapex = list(df.apex_frame.values)
    ...
    while len(tscore) > 0 and len(rscore) < opt['nms_top_K']:
        ...
        for idx in range(0, len(tscore)):  # 计算其与所有proposal的iou, 如果有proposal和它重叠率高，之后就不考虑它
            if idx != max_index:
...
def iou_for_find(df, opt):
    ...
    gt_apex_indices = tmp_anno_df.apex_frame.values // opt["RATIO_SCALE"]

    predict_start_indices = df.start_frame.values
    predict_end_indices = df.end_frame.values
    predict_apex_indices = df.apex_frame.values

    prepare_start = copy.deepcopy(predict_start_indices)
    prepare_end = copy.deepcopy(predict_end_indices)
    prepare_apex = copy.deepcopy(predict_apex_indices)

    if opt['dataset'] == 'cas(me)^2':
        gt_start_indices -= 1
        gt_end_indices -= 1

    tiou = np.array([0.] * len(df))
    twiou = np.array([0.] * len(df))
    w = np.array([0] * len(df))
    apex_diff = np.array([0.] * len(df))
    ...
    for j in range(len(gt_start_indices)):
        ...
        ious = iou_with_anchors(gt_start, gt_end, prepare_start, prepare_end)
        max_iou = max(ious)
        if max_iou >= 0.5:
            ...
            apex_diff_frame = prepare_apex[tmp_idx]
            gt_apex = gt_apex_indices[j]

            prepare_start[tmp_idx] = 0
            prepare_end[tmp_idx] = 0

            tiou[tmp_idx] = max_iou
            twiou[tmp_idx] = max_iou * (gt_end - gt_start + 1)
            w[tmp_idx] = gt_end - gt_start + 1
            apex_diff[tmp_idx] = abs(apex_diff_frame - gt_apex)/(gt_end - gt_start + 1)
